...
flags.DEFINE_string("metric", "mlp", "whether use contrastive or triple loss or classifier")
...
flags.DEFINE_string("vocab_filename", "bert.vocab", "vocab filename")
flags.DEFINE_string("src_bizdate", "zipfan5000", "the sigature for source dataset")
flags.DEFINE_string("bizdate", "test_finetune", "the date of running experiments")

if FLAGS.src_bizdate is None:
    raise ValueError("src_bizdate is required..")
...
def model_fn(features, mode, bert_config, vocab, init_checkpoint, learning_rate,
    ...
    if FLAGS.metric == "contrast":
        loss = get_contrastive_loss(src_embeddings, dst_embeddings)

    elif FLAGS.metric == "mlp":
        loss = get_classifier_cross_entropy(src_embeddings, dst_embeddings)
    else:
        raise Exception("please choose right metric..")
...
def get_contrastive_loss(src_embeddings, dst_embeddings):

    """Get loss and log probs for the contrastive pair"""
    # sequence_shape = modeling.get_shape_list(src_embeddings, expected_rank=2)
    # batch_size = sequence_shape[0]
    # hidden_size = sequence_shape[1]

    logits = tf.matmul(src_embeddings, tf.transpose(dst_embeddings))
    logits = tf.nn.log_softmax(logits, 1)
    # only diag is the positive sample
    per_example_loss = -1 * tf.diag_part(logits)
    loss = tf.reduce_mean(per_example_loss)

    return loss


def get_triplet_loss(src_embeddings, dst_embeddings):
    ...
    src_dot = tf.matmul(src_embeddings, tf.transpose(src_embeddings))
    src_square = tf.expand_dims(tf.diag_part(src_dot), axis=1)
    src_square = tf.tile(src_square, [1, FLAGS.batch_size])

    dst_dot = tf.matmul(dst_embeddings, tf.transpose(dst_embeddings))
    dst_square = tf.expand_dims(tf.diag_part(dst_dot), axis=0)
    dst_square = tf.tile(dst_square, [FLAGS.batch_size, 1])

    src_dot_dst = tf.matmul(src_embeddings, tf.transpose(dst_embeddings))

    euclidean_matrix = src_square - 2*src_dot_dst + dst_square

    pos_euclidean_distance = tf.diag_part(euclidean_matrix)
    pos_euclidean_matrix = tf.tile(tf.expand_dims(pos_euclidean_distance, axis=1), [1, FLAGS.batch_size])

    triple_loss = euclidean_matrix - pos_euclidean_matrix + margin

    # mask the positive part
    mask = 1 - tf.diag(tf.ones([FLAGS.batch_size]))
    triple_loss = triple_loss * mask
    # reduce easy loss
    triple_loss = tf.nn.relu(triple_loss)
    # triple_loss = tf.reshape(triple_loss, [-1])
    num_positive_losses = tf.reduce_sum(tf.cast(triple_loss > eps, tf.float32))
    loss = tf.reduce_sum(triple_loss) / (num_positive_losses + eps)
...
def run_finetune():
    ...
    input_file_name = "./data/eth." + FLAGS.strategy + ".pair." + FLAGS.bizdate
