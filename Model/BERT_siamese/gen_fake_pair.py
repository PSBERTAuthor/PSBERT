...
flags.DEFINE_string("strategy", None, "Strategy for fake pair generation")
flags.DEFINE_string("frequency", "freq", "Strategy for masking")
flags.DEFINE_string("bizdate", None, "the date of running experiments")
flags.DEFINE_string("src_bizdate", None, "the sigature for source dataset")
...
def gen_masked_samples(sequences):
    instances = []
    # create train
    start = time.time()
    for tokens in sequences:
...
def write_instance_to_example_files(instance_pairs, max_seq_length,
    ...
    def parser(instance, flag):
        ...
        features[flag + "_input_values"] = create_float_feature(values)
...
def difftime_sequence_split():
    ...
    # load sequence first
    print("===========Load Sequence===========")
    with open("../BERT/data/eoa2seq_" + FLAGS.src_bizdate + ".pkl", "rb") as f:
        eoa2seq = pkl.load(f)
...
def main():
    ...
    print("============Load Vocab==============")
    vocab_file_name = "../bert/data/" + FLAGS.vocab_filename + "." + FLAGS.src_bizdate
    ...
    # Write into Training instance中
    output_filename = FLAGS.data_dir + FLAGS.dataset_name+ "." + FLAGS.strategy + ".pair." + FLAGS.bizdate
