...
class TornadoQueries():
    def __init__(self, mixer_str_value="0.1", data_type="all", data_folder="../Data/tornado_dataset", max_time=None, verbose=True):
...
def load_embedding():
    ...
    if FLAGS.algo == "PSBERT":
        embeddings = np.load("PSBERT/data/embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load("PSBERT/data/address_" + FLAGS.model_index + ".npy")

    elif FLAGS.algo == "PSBERT_finetune":
        embeddings = np.load("PSBERT_finetune/data/embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load("PSBERT_finetune/data/address_" + FLAGS.model_index + ".npy")
...
def main():
    ...
    # tornado_address_set = set(pd.read_csv("../Data/tornado_dataset/tornado_account.csv").address.values)
